...
def compute_token_merge_indices(
    ...
    # 🔥 encode full sequence (có special tokens)
    inputs = tokenizer(prompt, return_tensors="pt")
    input_ids = inputs["input_ids"][0]

    tokens = [tokenizer.decode([i]).lower() for i in input_ids]

    # remove spaces artifacts
    tokens = [t.strip() for t in tokens]

    if word_idx is None:
        word = word.lower().strip()

        # 🔥 special case cho BOS / EOS / PAD
        if word in tokens:
            merge_idxs = [i for i, t in enumerate(tokens) if t == word]
        else:
            # fallback: search bình thường
            search_tokens = [t.strip() for t in tokenizer.tokenize(word)]

            for i in range(len(tokens)):
                if tokens[i : i + len(search_tokens)] == search_tokens:
                    merge_idxs += list(range(i, i + len(search_tokens)))

        if not merge_idxs:
            raise ValueError(f"Search word {word} not found in tokens: {tokens}")

    else:
        merge_idxs.append(word_idx)

    return merge_idxs, word_idx
